...
void LaneDetector::processFrame() {
    ...
    while (true) {
        ...
        // ========== TÍNH CÁC THAM SỐ MPC ==========
        float curvature = 0.0f;
        float angle_y = 0.0f;
        float offset = 0.0f;
        
        mpc_computer_.computeMpcParameters(centerline, bird_eye_view,
                                          curvature, offset, angle_y);
        mpc_params_.updateParameters(curvature, offset, angle_y);

        // ========== HIỂN THỊ LÊN FRAME ==========
        // Tạo một vùng nền đen cho text
        cv::Rect textBox(10, 10, 320, 110);
        cv::rectangle(frame_resize, textBox, cv::Scalar(0, 0, 0), -1);
        cv::rectangle(frame_resize, textBox, cv::Scalar(0, 255, 255), 2);

        // Hiển thị curvature
        std::string text_curv = "Curvature: " + std::to_string(curvature) + " (1/m)";
        ...
        // Hiển thị offset
        std::string text_lat = "Offset: " + std::to_string(offset) + " (m)";
        ...
        // Hiển thị angle_y
        float yaw_degree = angle_y * 180.0f / M_PI; // Chuyển sang độ
