...
const float DISTANCE_TO_AXLE = 0.15f;  // 15cm từ đáy ảnh đến trục xe
...
std::vector<float> ComputeMpc::computeMultipleCurvatures(const cv::Vec3f& coeffs, int N) {
    ...
    for (int i = 0; i < N; ++i) {
        float y = vehicle_y_ - i * 26.0f;  // 26 pixel ≈ 3cm
