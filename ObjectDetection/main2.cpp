...
public:
    ...
    void processFrame() {
        ...
        while (true) {
            ...
            //Sliding Window with slope condition
            slidingWindow(mask, left_points, right_points, bird_eye_view, right_coeffs ,false);
            ...
            int y_mid = bird_eye_view.rows/2;
            float slope_right = computeLaneSlope(right_coeffs, y_mid);
            float slope_threshold = 0.2f;
            bool change_lane = (std::abs(slope_right) > slope_threshold);
            if (change_lane) {
                left_points.clear();
                right_points.clear();
                slidingWindow(mask, left_points, right_points, bird_eye_view, right_coeffs, true);
            }
            // ===== Tính centerline =====
    ...
    void slidingWindow(const cv::Mat& mask,
                        ...
                        cv::Mat& outImg, cv::Vec3f prev_poly, 
                        bool Flag_change_lane)
    {
        ...
        if(!Flag_change_lane) {
            ...
            hist = hist.reshape(1, hist.cols);  // Chuyển thành vector 1D để dễ xử lý

            // Tìm vị trí max của left và right lane
            int midpoint = hist.cols / 2;
            auto it_left = std::max_element(hist.begin<int>(), hist.begin<int>() + midpoint);
            auto it_right = std::max_element(hist.begin<int>() + midpoint, hist.end<int>());

            // Tính vị trí x_base của left và right
            int leftx_base = it_left - hist.begin<int>();
            int rightx_base = it_right - hist.begin<int>();
            int hist_threshold = mask.rows/50;
            if (*it_left < hist_threshold) { 
                leftx_base = -1; // Đặt giá trị không hợp lệ
            }
            if (*it_right < hist_threshold) { 
                rightx_base = -1; // Đặt giá trị không hợp lệ
            }
            int leftx_current, rightx_current;
            if(leftx_base != 1) leftx_current = leftx_base;
            if(rightx_base != 1) rightx_current = rightx_base;
            ...
            for (int window = 0; window < nwindows; window++) {
                ...
                bool left_base_ok = (leftx_base != -1);
                bool right_base_ok = (rightx_base != -1);
                if (left_base_ok) {
                    left_win = cv::Rect(leftx_current - margin, win_y_low, margin*2, window_height);
                }
                if (right_base_ok) {
                    right_win = cv::Rect(rightx_current - margin, win_y_low, margin*2, window_height);
                }

                std::vector<cv::Point> good_left, good_right;
                for (auto &p : nonzero) {
                    if (left_base_ok && left_win.contains(p))  good_left.push_back(p);
                    if (right_base_ok && right_win.contains(p)) good_right.push_back(p);
                }

                if (left_base_ok && !good_left.empty()) {
                    cv::rectangle(outImg, left_win, cv::Scalar(0,255,0), 2);
                }
                if (right_base_ok && !good_right.empty()) {
                ...
                if (left_base_ok && (int)good_left.size() > minpix) {
                    int sumx = 0;
                    for (auto &p : good_left) sumx += p.x;
                    leftx_current = sumx / (int)good_left.size();
                }
                if (right_base_ok && (int)good_right.size() > minpix) {
                    int sumx = 0;
