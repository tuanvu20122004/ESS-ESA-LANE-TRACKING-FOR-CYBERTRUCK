...
void LaneDetector::processFrame(cv::Mat& frame_resize) {
    ...
    if (!initialized) {
        ...
        left_ok  = (left_points.size()  >= 150);
        right_ok = (right_points.size() >= 150);
...
/*void LaneDetector::processFrame(cv::Mat& frame_resize) {
    ...
    if(left_ok && right_ok) {
        ...
        float lane_width = std::fabs(x_right - x_left);
        if (lane_width < 10) {
            if(x_left < bird_eye_view.cols / 2) {
                merging_right_flag = true;
            } else {
                merging_left_flag = true;
            }
    ...
    if (left_points.size()  >= 80 && merging_left_flag == false) {
        left_ok = true;
    ...
    if (right_points.size()  >= 80 && merging_right_flag == false) {
        right_ok = true;
    ...
    float slope_left = computeLaneSlope(left_coeffs, height - 1);
    float slope_right = computeLaneSlope(right_coeffs, height - 1);
    bool change_lane = (std::fabs(slope_left) > 0.5f) || (std::fabs(slope_right) > 0.5f);
    if (change_lane) minpix = 5;
    else minpix = 20;
...
// Copyright by Anle (origin version)
/*void LaneDetector::processFrame(cv::Mat& frame_resize) {
    bird_eye_view = applyIPM(frame_resize);
    mask = processMask(bird_eye_view);
...
std::vector<cv::Point> LaneDetector::computeCenterline(cv::Vec3f coeff_left,
    ...
    float current_laneW = 400.0f;
    if(laneW_avg < 390.0f || laneW_avg > 410.0f) {
        current_laneW = LANE_WIDTH_PX;
    }
    else current_laneW = laneW_avg;
