...
// ==================== MpcState Struct ====================
struct MpcState {
    std::vector<float> curvature;  // 10 phÃ¡ÂºÂ§n tÃ¡Â»Â­ (1/m)
...
public:
    MpcController()
        : wheelbase_(0.2515f),
          ...
          prev_z_dim_(-1),
          prev_constraint_dim_(-1),
          cached_velocity_(-1.0f) {}
    
    // KhÃ¡Â»Å¸i tÃ¡ÂºÂ¡o MPC vÃ¡Â»â€ºi weights
    ...
    // TÃƒÂ­nh gÃƒÂ³c lÃƒÂ¡i tÃ¡Â»Â« MpcState
    float computeSteeringAngle(const MpcState& state, float velocity) {
        ...
        if (std::abs(velocity - cached_velocity_) > 0.01f) {
...
private:
    ...
    int prev_z_dim_, prev_constraint_dim_;
    float cached_velocity_;
    ...
    float solveQP(const Eigen::VectorXd& x0, const Eigen::VectorXd& v_k) {
        ...
        Eigen::VectorXd lb = beq, ub = beq;

        if (!solver_initialized_ || prev_z_dim_ != z_dim || prev_constraint_dim_ != constraint_dim) {
            ...
            prev_z_dim_ = z_dim;
            prev_constraint_dim_ = constraint_dim;
        } else {
        ...
        u_cmd = std::clamp(u_cmd, umin_, umax_);
