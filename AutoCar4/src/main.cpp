...
int main(int argc, char* argv[]) {
    ...
    // ==================== AUTO CALIBRATION ====================
    cv::Mat first_frame;
    if (detector.getFrame(first_frame)) {
        detector.processFrame(first_frame);

        std::vector<cv::Point2f> src_points = detector.getSourcePoints();
     
        float real_lane_width_cm = 35.0f; 
        mpc.calibratePixelScale(src_points, real_lane_width_cm);
        
        // ✅ SYNC SCALE TO DETECTOR
        detector.setPixelScale(mpc.getPixelPerMeter());
        
        std::cout << "[MAIN] Auto calibration completed!" << std::endl;
        std::cout << "[MAIN] Current scale: " << mpc.getPixelPerMeter() << " pixels/meter" << std::endl;
        std::cout << "[MAIN]              = " << (mpc.getPixelPerMeter() / 100.0f) << " pixels/cm" << std::endl;
    }
    // ==========================================================
