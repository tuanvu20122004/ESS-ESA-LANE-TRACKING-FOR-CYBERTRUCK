...
void MpcController::calibratePixelScale(const std::vector<cv::Point2f>& src_points, 
                                        float real_lane_width_cm) {

    if (src_points.size() != 4) {
        std::cerr << "[MPC CALIBRATION] Error: Need exactly 4 source points!" << std::endl;
        pixel_per_meter_ = 1400.0f;
        return;
    }
    
    cv::Point2f tl = src_points[0];
    cv::Point2f bl = src_points[1];
    cv::Point2f tr = src_points[2];
    cv::Point2f br = src_points[3];
    
    float width_top = cv::norm(tr - tl);
    float width_bottom = cv::norm(br - bl);
    float width_mid = (width_top + width_bottom) / 2.0f;
    
    float length_left = cv::norm(bl - tl);
    float length_right = cv::norm(br - tr);
    float avg_length = (length_left + length_right) / 2.0f;
    
    float pixels_per_cm_width = width_mid / real_lane_width_cm;
    
    float assumed_length_cm = 100.0f;
    float real_area_cm2 = real_lane_width_cm * assumed_length_cm;
    float image_area_px2 = 0.5f * (width_top + width_bottom) * avg_length;
    float pixels_per_cm_area = std::sqrt(image_area_px2 / real_area_cm2);
    
    float pixels_per_cm = 0.6f * pixels_per_cm_width + 0.4f * pixels_per_cm_area;
    
    pixel_per_meter_ = pixels_per_cm * 100.0f;
    
    if (pixels_per_cm < 10.0f || pixels_per_cm > 20.0f) {
        pixels_per_cm = 14.0f;
        pixel_per_meter_ = 1400.0f;
    }
}
...
std::vector<float> MpcController::computeMultipleCurvatures(const cv::Vec3f& coeffs, int N) {
    ...
    float step_cm = 3.0f;
    float step_pixels = step_cm * (pixel_per_meter_ / 100.0f);

    for (int i = 0; i < N; ++i) {
        float y = vehicle_y_ - i * step_pixels;
...
float MpcController::computeYawAngle(const cv::Vec3f& coeffs, float y) {
    ...
    dx_dy = std::clamp(dx_dy, -10.0f, 10.0f);
